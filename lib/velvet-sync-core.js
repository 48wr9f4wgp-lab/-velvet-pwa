...
function uniqueStrings(values, max = 2000) {
  ...
  if (result.length > max) throw Object.assign(new Error("favorite_capacity_exceeded"), { statusCode: 409 });
  return result;
}
...
function makeFingerprints({ id, source, pageUrl, imageUrl }) {
  // Pages and legacy aliases do not establish image equality.
  const mediaIdentity = canonicalizeUrlForIdentity(imageUrl);
  if (mediaIdentity) return ["m:" + mediaIdentity];
  if (id && source) return ["s:" + source.toLowerCase() + ":" + id];
  if (id) return ["i:" + id];
  const pageIdentity = canonicalizeUrlForIdentity(pageUrl);
  return pageIdentity ? ["p:" + pageIdentity] : [];
}
...
export function normalizeFavorite(raw, sourceHint = "unknown") {
  ...
  const trustedAliases = raw.identity_version === 53 ? aliases : {};
  const aliasIds = uniqueStrings([...(trustedAliases.ids || []), id].filter(Boolean), 1000);
  ...
  const firstSaved = isoDate(raw.first_saved_at ?? raw.saved_at ?? raw.archived_at ?? raw.ts, new Date(0).toISOString());
  const lastSaved = isoDate(raw.last_saved_at ?? raw.saved_at ?? raw.archived_at ?? raw.ts, firstSaved);
  const item = {
    identity_version: 53,
    uid: clip(raw.uid, 8192) || null,
    legacy_aliases: raw.legacy_aliases || (raw.identity_version !== 53 && raw.aliases ? raw.aliases : null),
    ...
    aliases: { ids: aliasIds, page_urls: aliasPages, media_urls: aliasMedia, sources: aliasSources, fingerprints: [] },
    provenance: uniqueStrings([...(raw.provenance || []), ...(sourceHint && sourceHint !== "existing" ? [sourceHint] : [])], 32)
  };
  item.aliases.fingerprints = makeFingerprints({ id: item.id, source: item.source, pageUrl: item.page_url, imageUrl: item.image_url });
...
function quality(item) {
  return (item.page_url ? 4 : 0) + (item.image_url ? 4 : 0) + (item.title ? 1 : 0)
    + (item.source && item.source !== "unknown" ? 1 : 0) + (item.tags?.length ? 0.5 : 0);
}

function mergeGroup(group) {
  const sorted = [...group].sort((a, b) => quality(b) - quality(a) || String(b.last_saved_at || "").localeCompare(String(a.last_saved_at || "")));
  ...
  const firstSaved = sorted.map(item => item.first_saved_at).filter(Boolean).sort()[0] || new Date(0).toISOString();
  const lastSaved = sorted.map(item => item.last_saved_at).filter(Boolean).sort().at(-1) || firstSaved;
  return {
    ...primary, uid: chooseUid(aliases.fingerprints), id: primary.id || aliases.ids[0] || null,
    image_url: primary.image_url || "", page_url: primary.page_url || aliases.page_urls[0] || null,
    first_saved_at: firstSaved, last_saved_at: lastSaved, aliases,
...
export function unionFavorites(existing, incoming, sourceHint = "unknown") {
  ...
  for (const row of rows) {
    for (const id of row.aliases.ids) {
      if (!idOwners.has(id)) idOwners.set(id, new Set());
      idOwners.get(id).add(row.uid);
    }
  }
  const result = [...groups.values()].map(mergeGroup).map(item => {
    const safeIds = item.aliases.ids.filter(id => idOwners.get(id)?.size === 1);
    const ambiguous = idOwners.get(item.id)?.size > 1;
    return {
      ...item,
      id: ambiguous ? "vf53:" + createHash("sha256").update(item.uid).digest("hex") : item.id,
      aliases: { ...item.aliases, ids: safeIds }
    };
  }).sort((a, b) => String(b.last_saved_at || "").localeCompare(String(a.last_saved_at || "")) || String(a.uid).localeCompare(String(b.uid)));
  if (result.length > MAX_SYNC_FAVORITES) throw Object.assign(new Error("favorite_capacity_exceeded"), { statusCode: 409 });
  return result;
}

export function mergeSyncLibrary(current, payload, sourceHint = "unknown") {
  const currentItems = Array.isArray(current?.items) ? current.items : [];
  const incoming = Array.isArray(payload?.favorites) ? payload.favorites : Array.isArray(payload?.items) ? payload.items : [];
  ...
  const incomingLikedIds = uniqueStrings(payload?.likedIds ?? payload?.liked_ids ?? payload?.orphan_ids ?? [], MAX_SYNC_ORPHAN_IDS);
  const orphanIds = uniqueStrings([...currentOrphans, ...incomingLikedIds], MAX_SYNC_ORPHAN_IDS).filter(id => !representedIds.has(id));
  ...
  return {
    schema_version: SYNC_SCHEMA_VERSION, items, orphan_ids: orphanIds,
    changed: JSON.stringify(previousComparable) !== JSON.stringify(nextComparable),
    stats: {
      item_count: items.length, orphan_id_count: orphanIds.length,
      added_items: items.filter(item => !previousUids.has(item.uid)).length,
      incoming_favorites: incoming.length, incoming_liked_ids: incomingLikedIds.length
    }
  };
}
...
export function combineSyncStates(states) {
  ...
  let combined = { schema_version: SYNC_SCHEMA_VERSION, items: [], orphan_ids: [] };
  let revision = 0, updatedAt = null;
  for (const row of list) {
    ...
    const merged = mergeSyncLibrary(combined, {
      ...
      orphan_ids: Array.isArray(state.orphan_ids) ? state.orphan_ids : []
    }, source);
    combined = { schema_version: SYNC_SCHEMA_VERSION, items: merged.items, orphan_ids: merged.orphan_ids };
    revision += Math.max(0, Number(state.revision) || 0);
    if (state.updated_at && (!updatedAt || String(state.updated_at) > String(updatedAt))) updatedAt = state.updated_at;
  }
  return { schema_version: 2, identity_version: 53, revision, updated_at: updatedAt, items: combined.items, orphan_ids: combined.orphan_ids, add_only: true };
}
